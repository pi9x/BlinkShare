  ...
  template: `
    <section class="mx-auto max-w-2xl">
      <article class="surface-card rounded-[1.5rem] p-6">
        <h2 class="text-2xl font-semibold tracking-tight text-slate-950">Register</h2>
        ...
        <form class="mt-5 space-y-4" [formGroup]="form" (ngSubmit)="submit()">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <a class="app-button app-button-secondary" routerLink="/auth/login">Login</a>
          </div>
        </form>
      </article>
    </section>
...
export class RegisterPageComponent {
  ...
  protected async submit(): Promise<void> {
    ...
    try {
      await this.authService.register(this.form.getRawValue().email, this.form.getRawValue().password);
