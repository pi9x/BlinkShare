...
public sealed class Pbkdf2PasscodeHasher : IPasscodeHasher
{
    ...
    public string Hash(string passcode)
    {
        ...
        var key = Rfc2898DeriveBytes.Pbkdf2(passcode, salt, Iterations, HashAlgorithmName.SHA256, KeySize);
    ...
    public bool Verify(string passcode, string persistedHash)
    {
        ArgumentNullException.ThrowIfNull(passcode);
        ...
        try
        {
            ...
            var actualKey = Rfc2898DeriveBytes.Pbkdf2(passcode, salt, iterations, HashAlgorithmName.SHA256, expectedKey.Length);
